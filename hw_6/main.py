...
class MazeWindow:
    ...
    def plotBackground(self):
        for i, row in enumerate(self.maze.tolist()):
            for j, element in enumerate(row):
                bg = 'black' if element == 1 else 'yellow' if element == 2 else 'green' if element == -1 else 'white'
                self.labels[i][j] = tk.Label(self.root, foreground='blue', background=bg, width=2, height=1, relief='ridge', font='? 4 bold')
    ...
    def target(self, indexes):
        ...
        self.root.update()
    def showBestWay(self, QTable):
        environment = Environment()
        temp = start
        visited = {}
        while temp[0] != end[0] or temp[1] != end[1]:
            i,j = temp
            visited[(i,j)]=1
            self.labels[i][j] = tk.Label(self.root, foreground='blue', background='red', width=1, height=1, relief='ridge', font='? 4 bold')
            print('当前位置为',temp)
            Qa = QTable[i][j].tolist()
            qmap = {i:Qa[i] for i in range(4)}
            qa = sorted(qmap.items(), key = lambda kv:(kv[1], kv[0]), reverse=True)
            print(qa)
            for k in range(4):
                action = self.actionList[qa[k][0]]
                next,_ = environment.getNextState(temp,action)
                if maze[next[0]][next[1]]==0 and tuple(next) not in visited:
                    temp = next
                    break
                if k==3:
                    action = self.actionList[qa[0][0]]
                    next,_ = environment.getNextState(temp,action)
                    temp=next

        si,sj = start
        self.labels[si][sj] = tk.Label(self.root, foreground='blue', background='green', width=1, height=1, relief='ridge', font='? 4 bold')
...
class Agent:
    ...
    def showBestAction(self):
        for i, row in enumerate(self.QTable):
            ...
            print()
    def getAction(self, eGreddy=0.9):
